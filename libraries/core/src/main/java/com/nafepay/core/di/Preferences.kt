...
class Preferences constructor(
    ...
    suspend fun setAuthToken(token: String) {
        context.dataStore.edit { settings -> settings[KEY_AUTH_TOKEN] = token }
    }
    ...
    suspend fun setRefreshToken(token: String) {
        context.dataStore.edit { settings -> settings[KEY_REFRESH_TOKEN] = token }
    }

    private val ENCRYPTION_KEY = stringPreferencesKey("encryptionKey")
    val encryptionKey: Flow<String?> = context.dataStore.data
        .map { preferences -> preferences[ENCRYPTION_KEY] }

    suspend fun setEncryptionKey(key: String) {
        context.dataStore.edit { settings -> settings[ENCRYPTION_KEY] = key }
    }

    private val SOCIAL_MEDIA_LOGIN = booleanPreferencesKey("social_login")
    val socialLogin: Flow<Boolean> = context.dataStore.data
        .map { preferences -> preferences[SOCIAL_MEDIA_LOGIN] ?: false }

    suspend fun setSocialLogin(loggedIn: Boolean) {
        context.dataStore.edit { settings -> settings[SOCIAL_MEDIA_LOGIN] = loggedIn }
    }

    private val DARK_MODE = booleanPreferencesKey("isDarkMode")
    ...
    suspend fun switchDarkMode(isDarkMode: Boolean) {
        context.dataStore.edit { settings -> settings[DARK_MODE] = isDarkMode }
    }
