...
import com.nafepay.domain.models.authentication.ResendCodeRequest
...
class AuthenticationKtorService(private val client: HttpClient) {
    ...
    suspend fun getUserNotifications(userId:String,page: Int, unread: Boolean): NotificationsResult? {
        var url = "User/notifications/$userId?page=$page&size=${10}"
        if (unread)
            url =
                "User/notifications/unread/$userId?page=$page&size=${10}"
